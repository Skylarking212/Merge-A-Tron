    ...
    for (let i = 0; i < 5; i++) {
      ...
      positions.push(
        <div key={i} className="bg-white rounded-lg p-6 shadow mb-4">
          <h3 className="text-lg font-semibold text-red-800">Position {i + 1}</h3>
          {member ? (
            <div>
              <p className="font-medium">{member.username}</p>
              <p className="text-gray-600 mt-1">
                Skills: {member.skills && member.skills.length > 0 
                  ? member.skills.join(", ") 
                  : "No skills listed"}
              </p>
            </div>
      ...
      <div className="min-h-screen bg-sky-300">
        <header className="bg-red-800">
          <nav className="container mx-auto flex justify-around px-4 py-4">
            ...
            <NavTab label="TEAMS" isActive={true} onClick={() => router.push('/teams')} />
        ...
        <main className="container mx-auto px-4 py-10">
          <div className="bg-white rounded-xl p-8 shadow-lg mb-8">
            <h1 className="text-3xl font-bold text-red-800 mb-4">Error Loading Team</h1>
  ...
  return (
    <div className="min-h-screen bg-sky-300">
      ...
      <main className="container mx-auto px-4 py-10">
        ...
        <div className="bg-white rounded-xl p-8 shadow-lg mb-8">
          <h1 className="text-3xl font-bold text-red-800 mb-2">{team?.name || "Team"}</h1>
          <p className="text-gray-600">{team?.description || "No description available"}</p>
          <p className="mt-2 text-sm text-gray-500">Team ID: {team_id}</p>
        </div>

        <h2 className="text-2xl font-bold text-red-800 mb-6">Team Members</h2>
